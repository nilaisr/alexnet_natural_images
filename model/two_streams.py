...
def conv2D_bn(x, filters, num_row, num_col, strides=(1, 1), padding='same', weights_init='RandomUniform', name=None):
    ...
    Arguments:
        ...
        weights_init: initialisation weights.
    ...
    """
    bn_axis = 3
    x = Conv2D(filters, (num_row, num_col), strides=strides, padding=padding, kernel_initializer=weights_init)(x)
...
def two_streams():
    num_classes = 20#444  # number of classes
    dropout = 0.5

    data_format = 'channels_last'
    input_shape = (224, 224, 3)  # 3 - Number of RGB Colours

    weights_init = 'RandomUniform'
    ...
    x = Conv2D(192, (3, 3), strides=(1, 1), padding='same', kernel_initializer=weights_init)(x)

    # Channel 2 - Conv Net Layer 3
    y = Conv2D(192, (3, 3), strides=(1, 1), padding='same', kernel_initializer=weights_init)(y)

    # Channel 1 - Conv Net Layer 4
    x1 = Concatenate()([x, y])
    x1 = Conv2D(192, (3, 3), strides=(1, 1), padding='same', kernel_initializer=weights_init)(x1)

    # Channel 2 - Conv Net Layer 4
    y1 = Concatenate()([x, y])
    y1 = Conv2D(192, (3, 3), strides=(1, 1), padding='same', kernel_initializer=weights_init)(y1)

    # Channel 1 - Conv Net Layer 5
    x2 = Concatenate()([x1, y1])
    x2 = Conv2D(128, (3, 3), strides=(1, 1), padding='same', kernel_initializer=weights_init)(x2)
    x2 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), data_format=data_format)(x2)

    # Channel 2 - Conv Net Layer 5
    y2 = Concatenate()([x1, y1])
    y2 = Conv2D(128, (3, 3), strides=(1, 1), padding='same', kernel_initializer=weights_init)(y2)
    y2 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), data_format=data_format)(y2)

    # # Channel 1 - Cov Net Layer 6
    x3 = Multiply()([x2, y2])
    x3 = Flatten()(x3)
    x3 = Dense(2048, activation='relu', kernel_initializer=weights_init)(x3)
    x3 = Dropout(dropout)(x3)

    # Channel 2 - Cov Net Layer 6
    y3 = Multiply()([x2, y2])
    y3 = Flatten()(y3)
    y3 = Dense(2048, activation='relu', kernel_initializer=weights_init)(y3)
    y3 = Dropout(dropout)(y3)

    # Channel 1 - Cov Net Layer 7
    x4 = Multiply()([x3, y3])
    x4 = Dense(2048, activation='relu', kernel_initializer=weights_init)(x4)
    x4 = Dropout(dropout)(x4)

    # Channel 2 - Cov Net Layer 7
    y4 = Multiply()([x3, y3])
    y4 = Dense(2048, activation='relu', kernel_initializer=weights_init)(y4)
    y4 = Dropout(dropout)(y4)

    # Final Channel - Cov Net 9
    xy = Multiply()([x4, y4])
    output = Dense(num_classes, activation='softmax', kernel_initializer=weights_init)(xy)
